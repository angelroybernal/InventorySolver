Public Class frmModulo2

    'Llama al procedimiento que carga los nombres de los parametros de entrada y salida
    Private Sub frmModulo2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        CargarM2() 'Llamado al procedimiento que carga los nombres de los parametros de entrada y salida
        btnResolver.Enabled = False
        btnGráfica.Visible = False
    End Sub

    'Llama al procedimiento que retorna a la pantalla de menu
    Private Sub FrmModulo2_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Salir()
    End Sub
    ...
    Private Sub BorrarTodoToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles BorrarTodoToolStripMenuItem.Click
        LimpiarM2() 'Llamado al procedimiento de limpieza
        btnResolver.Enabled = False
        btnGráfica.Visible = False
    ...
    Private Sub dgwEntrada_RowValidating(sender As Object, e As DataGridViewCellCancelEventArgs) Handles dgwEntrada.RowValidating
        If dgwEntrada.Item(1, 5).Selected = True And Not (Format(dgwEntrada.Item(1, 5).Value, Style:="") = "" Or dgwEntrada.Item(1, 5).Value = Format(0, Style:="")) Then
            dgwEntrada.Item(1, 6).Value = dgwEntrada.Item(1, 0).Value / dgwEntrada.Item(1, 5).Value
        ElseIf dgwEntrada.Item(1, 6).Selected = True And Not (Format(dgwEntrada.Item(1, 6).Value, Style:="") = "" Or dgwEntrada.Item(1, 6).Value = Format(0, Style:="")) Then
            dgwEntrada.Item(1, 5).Value = dgwEntrada.Item(1, 0).Value / dgwEntrada.Item(1, 6).Value
        End If
        ...
        If Not (Format(dgwEntrada.Item(1, 0).Value, Style:="") = "" Or dgwEntrada.Item(1, 0).Value = Format(0, Style:="")) And
           ...
           Not (Format(dgwEntrada.Item(1, 5).Value, Style:="") = "" Or dgwEntrada.Item(1, 5).Value = Format(0, Style:="")) And
